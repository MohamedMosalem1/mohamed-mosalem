               ...
               if ( $email_1 == $row['email'] ) {
                  if ( $hashed_pass == $row['password'] ) {
                     if ( $row['status'] == 'user') {
                        ...
                        $message[] = "You are now logged in." . $row['name'];
                        header('location:index.php');
                        exit();
...
if( isset($_REQUEST['register']) && $_SERVER['REQUEST_METHOD'] === 'POST' && $_REQUEST['register'] ){
   if (isset( $_POST['name'], $_POST['email'], $_POST['pass'], $_POST['cpass'])
      ...
      } elseif ( $pass_hash != $cpass_hash ) {
         $message[] = 'Password and Confirm Password do not match!';
      } elseif ( $pass_hash == $cpass_hash ) {
         $message[] = 'Password and Confirm Password match!';
      } else {
         $message[] = 'Access denied: errors not found here!';
      }
      if ( empty($message) ) {
         try {
            $select_user = $conn->prepare("SELECT * FROM `admin and users` WHERE name = ? AND email = ?");
            ...
            if( $select_user -> rowCount() > 0 ){
               ...
               if ( $pass != $cpass ){
                  ...
                  $insert_user->execute([$name, $email, $cpass, 'user']);
